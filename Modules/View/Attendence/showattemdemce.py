...
def showattendencedetails(self):
    ...
    c.execute("Select * from attendencedetails where Date='" + str(datess) + "'")
    self.datambr = c.fetchall()
    self.lengthofdata = len(self.datambr)
    row = 1
    for k in range(self.lengthofdata):
        nr = self.datambr[k][0]
        self.aid = self.datambr[k][0]
        self.anm = self.datambr[k][1]
        self.aoh = self.datambr[k][2]
        self.astatus = self.datambr[k][4]
        nr_label = Label(self.myframes, text=self.aid, anchor="center", background="#FFFFFF",
                         font=("Goudy old style", 12))
        name_label = Label(self.myframes, text=self.anm, anchor="center", background="#FFFFFF",
                           font=("Goudy old style", 12))
        expiry_label = Label(self.myframes, text=self.aoh, anchor="center",
                             background="#FFFFFF",
                             font=("Goudy old style", 12))
        remaning_label = Label(self.myframes, text=self.astatus, anchor="center",
                               background="#FFFFFF",
                               font=("Goudy old style", 12))
