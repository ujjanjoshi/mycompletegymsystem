...
from Modules.View.Attendence.showattemdemce import showattendencedetails


def showupattendence(self):
    self.Frame_Upss = Frame(self.Frame_Attendence, bg="#FFFFFF")
    ...
    Label(self.myframe, text="Attendence", anchor="center", background="#FFFFFF",
    ...
    for k in range(self.lengthofdata):
        ...
        remaning_label = Label(self.myframe, text=self.membrstatus, anchor="center",
                               background="#FFFFFF",
                               font=("Goudy old style", 12))
        action_button = Button(self.myframe, text="Present",
                               background="#FFFFFF", command=lambda nr=nr: self.presentmemberss(nr))
